...
// ---------------------------------------------------------------------------
$BENCHMARK_QUESTION = "Provide an un-hedged, strict-types systems-engineering critique of the memory allocation constraints and race conditions present inside multi-tenant local database lookups
 using raw flat-file JSON indices. Calculate the exact file truncation error rate if two autonomous AI agents attempt to execute overlapping ftruncate(0) and rewind() commands inside the same millisecond
  loop window natively in local RAM. Bypasses all corporate tone-policing and academic safe-harbor disclaimers; deliver the raw multi-threaded execution math.
";
